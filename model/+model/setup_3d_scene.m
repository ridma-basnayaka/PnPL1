...
%             seglen = segLenPS(i)*randn;
            ...
            while (~gend)
                ...
                Xdir = Xd(:, lineInd);
                Xdir = Xdir / norm(Xdir); % 方向向量                               

                % 模型直线（相机坐标系）
                segLen = (rand+0.5)*segLenBig;%*0.75*(rand+1); % [0,1.5*segLenBig]
                segShift = segLenShiftScale*(2*rand-1)*0.5*Xdir; 
                Xm1 = Xm + segShift; % 模型直线中点坐标
                Xp1 = Xm1 + 0.5*Xdir*segLen;
                Xp2 = Xm1 - 0.5*Xdir*segLen;              
                XL1(:, lineInd) = Xp1;
                XL2(:, lineInd) = Xp2;                    
    
                % 相机坐标系中模型直线端点沿着线移动，用于投影到图像平面
                % 具体实现是用不同的 segShift 
                segShift = segLenShiftScale*(2*rand-1)*0.5*Xdir;
                Xm2 = Xm + segShift;
                if (is_len_var) % 是否保持直线长度不变，默认是
        ...
        XLTw = {inv(R)*(XLT1-repmat(t,1,nlines)), inv(R)*(XLT2-repmat(t,1,nlines))}; % 图像直线端点在世界坐标系中对应的点坐标
