function [K,W,V] = EOPnPL_Pm_compute_K(Xw,Ximgn,xs,xe,Xs,Xe)
% 输入参数要求
...
if (isempty(Xw) && isempty(Ximgn))
    ...
    V(1:3:3*n-2,:) = -[I(:,1).*(Xs') I(:,2).*(Xs') I(:,3).*(Xs')];
    V(2:3:3*n-1,:) = -[I(:,1).*(Xe') I(:,2).*(Xe') I(:,3).*(Xe')];
    V(3:3:3*n  ,:) = -2*[I(:,1).*(Xm') I(:,2).*(Xm') I(:,3).*(Xm')];
