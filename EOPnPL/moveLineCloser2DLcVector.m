...
% 输入 Xsi, Xei, Lc 是findBestRTReproj函数对应输出 Xsf、Xef、Lcf
% 输出 Xsn、Xen 是相机坐标系坐标，函数外应转换为世界坐标系坐标

    LineSpt = -Lc(1:2, :).*repmat(Lc(3, :), 2, 1);
    Ld = [-Lc(2, :); Lc(1, :)]; % 图像直线斜率方向，Lc(1:2,:)是法线方向
    xd = xe - xs;
    ...
    xsc = (xs+xe)/2;
    p = Ld(1,:).*xsc(1,:) + Ld(2, :).*xsc(2, :); % 文章里的式5推导得出的式子，里面的p似乎为0
    Sol = p-SegLen/2; % 文章式5里的γ
    Sol2 = repmat(Sol, 2, 1);
    xsn = LineSpt + Ld.*Sol2;
