function [s] = damped_newton(s0,K,W,V,Xw,Ximgn,A)
...
s = s0;
for i = 1 : n
%     cost_pre(i) = cost_function_val(s(:,i),K);
    [Re,te] = s2Rt(s(:,i),W,V);
    [err_pre(i)] = reprojection_error_usingRT(Xw',Ximgn',Re,te,A);
end
...
%         cost_pre(i) = cost_function_val(s(:,i),K);
        [Re,te] = s2Rt(s(:,j),W,V);
        [err(:,j)] = reprojection_error_usingRT(Xw',Ximgn',Re,te,A);
    end
...
%      h13*vec h23*vec h33*vec ];
%  
%     d = vpa(( H + lambda * eye )\G);
%     s = vpa(s - d);
% %     cost = cost_function_val(s,K);
%     [Re,te] = s2Rt(s,W,V);
%     [err] = reprojection_error_usingRT(Xw',Ximg',Re,te,A);
% 
